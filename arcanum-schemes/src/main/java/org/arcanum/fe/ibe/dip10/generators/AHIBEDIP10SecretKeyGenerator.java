...
public class AHIBEDIP10SecretKeyGenerator implements CipherParametersGenerator {
    ...
    public CipherParameters generateKey() {
        if (params instanceof AHIBEDIP10SecretKeyGenerationParameters) {
            ...
            // K11
            Element K11 = pk.getY1().powZn(r1).mul(ElementUtils.randomIn(pairing.getZr(), pk.getY3()));
            ...
            K12.mul(msk.getX1())
              ...
              .mul(ElementUtils.randomIn(pairing.getZr(), pk.getY3()));

            // E1s
            Element[] E1s = new Element[uNum - idNum];
            for (int i = 0; i < E1s.length; i++) {
                E1s[i] = pk.getUAt(idNum + i).powZn(r1).mul(ElementUtils.randomIn(pairing.getZr(), pk.getY3())).getImmutable();
            }

            // K21
            Element K21 = pk.getY1().powZn(r2).mul(ElementUtils.randomIn(pairing.getZr(), pk.getY3()));
            ...
            for (int i = 0; i < E2s.length; i++) {
                E2s[i] = pk.getUAt(idNum + i).powZn(r2).mul(ElementUtils.randomIn(pairing.getZr(), pk.getY3())).getImmutable();
            }
            ...
            for (int i = 0; i < E1s.length; i++) {
                E1s[i] = sk.getE1At(i+1)
                        ...
                        .mul(ElementUtils.randomIn(pairing.getZr(), pk.getY3())).getImmutable();
            }
