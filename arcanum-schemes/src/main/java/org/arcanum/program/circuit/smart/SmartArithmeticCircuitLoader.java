package org.arcanum.program.circuit.smart;

import org.arcanum.Element;
import org.arcanum.Field;
import org.arcanum.common.io.IOUtils;
import org.arcanum.program.circuit.ArithmeticCircuit;
import org.arcanum.program.circuit.ArithmeticCircuit.ArithmeticCircuitGate;
import org.arcanum.program.circuit.Gate;
...
import static org.arcanum.program.circuit.Gate.Type.INPUT;

/**
 * @author Angelo De Caro (arcanumlib@gmail.com)
 */
public class SmartArithmeticCircuitLoader {

    public ArithmeticCircuit load(Field Zq, String path) {
        try {
...
//            System.out.println("n3 = " + n3);
            ...
            ArithmeticCircuit circuit = new ArithmeticCircuit(n1 + n2, nGates, nWires, n3);
            for (int i = 0; i < circuit.getNumInputs(); i++) {
                circuit.addGate(new ArithmeticCircuitGate(INPUT, i));
            }
            ...
            while (true) {
                ...
                int nCoeffs = Integer.valueOf(st.nextToken());
                ...
                Element[] coeffs = new Element[nCoeffs];
                for (int i = 0; i < nCoeffs; i++)
                    coeffs[i] = Zq.newElement(new BigInteger(st.nextToken()));


                Gate.Type type = Gate.Type.valueOf(st.nextToken());

                circuit.addGate(new ArithmeticCircuitGate(type, ins, outs, coeffs));
            }
